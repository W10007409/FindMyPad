...
AA:BB:CC:DD:EE:01,본관,3,동측,회의실 앞`}
      ...
      <label className="block text-sm text-fg">CSV
        <textarea value={csv} onChange={(e) => setCsv(e.target.value)} rows={10}
          className="mt-1 w-full rounded border border-border bg-surface p-2 font-mono text-sm text-fg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent motion-safe:transition-colors" placeholder="bssid,building,floor,zone,note" /></label>
      <button onClick={() => upload.mutate(csv)} disabled={!csv || upload.isPending}
        className="mt-3 rounded bg-accent px-4 py-2 text-white hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent disabled:opacity-50 motion-safe:transition">업로드</button>
      {upload.isSuccess && <p className="mt-3 text-ok">{upload.data.upserted}건 업서트됨</p>}
      {upload.isError && <p className="mt-3 text-danger">업로드 실패</p>}
    </div>
  );
}
