...
const item: DeviceListItem = { id: 1, serial: 'S1', assetNo: 'A-1', model: 'M', batteryPct: 50, lastSeenAt: '2026-07-13T00:00:00Z', lat: null, lng: null, currentUser: { empNo: 'E1', name: '홍길동', dept: '개발' }, indoor: { building: '본관', floor: '3', zone: '동측' }, org1: null, location: null, enrolled: true };
test('DeviceCard shows current user and serial', () => {
  render(<DeviceCard item={item} onClick={() => {}} />);
  expect(screen.getByText('홍길동')).toBeInTheDocument();
  expect(screen.getByText(/S1/)).toBeInTheDocument();
});
test('DeviceCard shows 미등록 chip when enrolled is false', () => {
  const unenrolled: DeviceListItem = { ...item, id: null, currentUser: null, enrolled: false };
  render(<DeviceCard item={unenrolled} onClick={() => {}} />);
  expect(screen.getByText('미등록')).toBeInTheDocument();
});
test('DeviceCard renders owner name when currentUser present', () => {
  render(<DeviceCard item={item} onClick={() => {}} />);
  expect(screen.getByText('홍길동')).toBeInTheDocument();
});
